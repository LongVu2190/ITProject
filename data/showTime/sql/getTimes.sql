...
WHERE (st.[showingDate] = CAST(GETDATE() AS Date) AND st.[startTime] > CONVERT(TIME, GETDATE()) AND mv.movieId = @movieId)
   OR (st.[showingDate] > CAST(GETDATE() AS Date) AND mv.movieId = @movieId)
